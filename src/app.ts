...
export class App {
  ...
  private async processMessage(msg: FetchedMessage): Promise<void> {
    ...
    const matchedAccount = targetRecipients
      .map((addr: string): AccountEntry | undefined =>
        this.accountsService.findAccountByEmail(addr),
      )
    ...
    const { aiReply, manualTrigger, noReply } =
      await this.replyService.generateReply({
        content,
    ...
    if (noReply) {
      console.log(
        "[MAIL] AI decided this email does not need a reply. Skipping.",
      );
      return;
    }
  ...
  private async processActionBatch(): Promise<void> {
    ...
    try {
      ...
      process.stderr.write(
        `[ACTION] Starting batch processing with lastId=${lastProcessedId}`,
      );
      ...
      if (latestSeenUid !== lastProcessedId) {
        await this.lastIdService.write(latestSeenUid);
        process.stderr.write(
          `[ACTION] Updated lastId to UID ${latestSeenUid}.`,
        );
      } else {
  ...
  async init(options?: { actionMode?: boolean }): Promise<void> {
    ...
    if (options?.actionMode) {
      process.stderr.write(
        "[ACTION] Running in --action mode (no IMAP listener).\n",
      );
