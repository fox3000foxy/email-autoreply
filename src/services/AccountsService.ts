...
export class AccountsService {
  ...
  private loadAccounts(): AccountEntry[] {
    ...
    const parsed = JSON.parse(raw) as Array<{ name?: string; email?: unknown; prompt?: string } & Record<string, unknown>>;
    ...
    return parsed.map((entry) => {
      ...
      if (rawEmail instanceof RegExp) {
        email = rawEmail;
      } else if (typeof rawEmail === 'string' && rawEmail.startsWith('regex:')) {
