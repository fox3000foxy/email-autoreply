...
export class ImapService {
    ...
    private persistentListeners: Array<{ event: string; handler: (...args: any[]) => void }> = [];
        ...
        while (true) {
            ...
            try {
                ...
                noopInterval = setInterval(() => {
                    ...
                    client.noop().catch(() => { /* will be caught by IDLE error */ });
                }, NOOP_INTERVAL_MS);
                ...
                console.error(`[IMAP] Connection error, reconnecting in ${delay / 1000}s`, err);
            } finally {
                // Cleanup
                ...
                try { await this.destroyClient(); } catch { /* best-effort */ }
            }
